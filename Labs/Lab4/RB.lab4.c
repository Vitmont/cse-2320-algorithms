...
char *currString, *outputString;
int length = 0, currlen = 0;
...
// Checks to see if invalid characters in tree string
void errorCheck(char *str)
{
  char *temp = str;
  for (temp = str; *temp != '\0'; temp++)
  {
    if (isdigit(*temp) == 0 && *temp != '+' && *temp != '-' && *temp != '.' && *temp != 'r' && *temp != 'b')
    {
      printf("Invalid Character %c found... Exiting\n", *temp);
      exit(0);
    }
  }
}

// Recursive deserialization call to turn string into RB tree
link deserialize()
{
  int len = strcspn(currString, "rb.") + 1;
  currlen += len;
  char *temp = malloc(len);
  strncpy(temp, currString, len);
  errorCheck(temp);
  currString += len;
  if (strcmp(temp, ".") == 0)
  {
  ...
  strncpy(num, temp, len - 1);
  link tempLink = NEW(atoi(num), 0, 0, 0);
  ...
  tempLink->l = deserialize();
  tempLink->r = deserialize();
  free(temp);
  free(num);
  return tempLink;
}

// Driver for deserialize also cleans up tree when done
...
// Recursive serialization call to turn tree into a string, frees tree in the proccess
void serialize(link head)
{
  if (head == z)
  {
    strcat(outputString, ".");
  ...
  char snum[10];
  sprintf(snum, "%d", head->item);
  strcat(outputString, snum);
  if (head->red == 1)
  {
...
// Driver for serialize, also frees the sentinal
char *STserialize()
{
  outputString = malloc(500);
  serialize(head);
  free(z);
  return outputString;
}

// Checks the length of the string to see if it is past the specified bytes
void checkLength(int bytes, char *str)
{
  if (strlen(str) > bytes)
  {
    printf("Warning: Length goes past given bytes");
  }
}